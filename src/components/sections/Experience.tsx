...
export function Experience() {
  ...
  return (
    <section id="experience" ref={sectionRef} className="py-24 relative z-10 bg-[#FCEDFD]">
      <div className="container mx-auto px-6 max-w-5xl">
        <div className="text-center mb-20">
          <h2 className="text-4xl md:text-5xl font-bold mb-4 text-[#1F0822]">Work <span className="text-[--color-accent-1]">Experience</span></h2>
          <p className="text-[#1F0822]/70 max-w-2xl mx-auto text-lg">
        ...
        <div className="relative">
          {/* Vertical Line */}
          <div className="absolute left-0 md:left-1/2 md:-translate-x-1/2 top-0 bottom-0 w-1 bg-[#1F0822]/10 rounded-full">
          ...
          <div className="space-y-12">
            {experiences.map((exp, index) => (
              ...
              >
                {/* Timeline Dot */}
                <div className="timeline-dot opacity-0 absolute left-[-28px] md:left-1/2 md:-translate-x-1/2 w-14 h-14 rounded-full bg-[#FCEDFD] border-4 border-[--color-accent-1] flex items-center justify-center z-10 shadow-[0_0_20px_rgba(236,105,250,0.5)]">
                  <Briefcase size={20} className="text-[--color-accent-1]" />
                </div>

                {/* Content */}
                <div className="timeline-item opacity-0 w-full md:w-1/2 pl-12 md:pl-0">
                  <div className={`bg-white/60 backdrop-blur-md border border-[#1F0822]/10 p-8 rounded-2xl group transition-transform duration-500 hover:-translate-y-2 hover:shadow-[0_20px_40px_-15px_rgba(236,105,250,0.3)] ${index % 2 === 0 ? "md:ml-12 text-left" : "md:mr-12 text-left md:text-right"}`}>
                    <div className={`flex flex-col gap-2 mb-6 ${index % 2 === 0 ? "items-start" : "items-start md:items-end"}`}>
                      <h3 className="text-2xl font-bold text-[#1F0822]">{exp.role}</h3>
                      <h4 className="text-xl text-[--color-accent-1] font-bold">{exp.company}</h4>
                      <div className="flex items-center gap-2 text-sm text-[#1F0822]/70 font-mono bg-[#1F0822]/5 px-3 py-1 rounded-full border border-[#1F0822]/10 mt-2">
                        <Calendar size={14} />
                        {exp.date}
                      </div>
                    </div>
                    
                    <ul className={`space-y-3 text-[#1F0822]/80 text-sm md:text-base font-medium ${index % 2 === 0 ? "" : "md:list-inside"}`}>
                      {exp.points.map((point, i) => (
                        <li key={i} className="flex gap-3">
                          <span className="text-[--color-accent-1] mt-1.5 font-bold">•</span>
